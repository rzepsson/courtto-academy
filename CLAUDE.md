...
- **Animations:** `motion-v` via the `motion-v/nuxt` module (`Motion`, `MotionConfig`, … are auto-registered). A global `<MotionConfig reduced-motion="user">` wraps the app in `app.vue`, so all motion honors the OS reduced-motion setting. Two tiers:
  ...
  - **Entrance animations on the dashboard (client area) only**, via `components/MotionReveal.vue` (fade + rise; `delay` prop staggers siblings). **Never on auth/SSR pages** (`layouts/auth.vue` and its pages render instantly — no entrance motion there). No page/layout transitions (disabled in `nuxt.config.ts`); use `<NuxtLoadingIndicator>` (in `app.vue`) for route-change feedback and `MotionReveal` for content.
